...
// Corrections and extensions provided by CH
...
public class SceneReference : ISerializationCallbackReceiver
{
	...
	public string SceneName {
		...
		set {
			sceneName = value;
#if UNITY_EDITOR
			// get spooky: apply reference. This allows runtime-set units to work in editor.
			string[] guids = AssetDatabase.FindAssets(sceneName+" t:Scene", null);
			// find close match
			string match = null;
			...
			for (int i = 0; i < guids.Length; i++)
			{
				...
				pathParts = assetPath.Split(char.Parse("/"));
				// Debug.Log(assetPath);
				if (pathParts[pathParts.Length - 1] == sceneName+".unity")
				{
					match = assetPath;
				}
			}

			// if one, and only one, apply.
			if (match!=null)
			{
				sceneAsset = AssetDatabase.LoadAssetAtPath<Object>(match);
			}
#endif
		}
	}
